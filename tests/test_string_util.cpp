...
SCENARIO("split provides Python-like splitting of strings", "[string_util]") {
    ...
    SECTION("splitting string into pre-allocated vector") {
        phycoeval::str_util::split("Test_string__TO_Split", '_', elements);
    ...
    SECTION("splitting string and returning elements in a new vector") {
        std::vector<std::string> words = phycoeval::str_util::split("Test_string__TO_Split", '_');
    ...
    SECTION("splitting string by tabs") {
        phycoeval::str_util::split("Test\tstring\t\tTO\tSplit", '\t', elements);
        REQUIRE(elements.size() == 5);
        REQUIRE(elements[0] == "Test");
        REQUIRE(elements[1] == "string");
        REQUIRE(elements[2] == "");
...
TEST_CASE("Testing pad_int", "[string_util]") {
    SECTION("Testing pad_int") {
        REQUIRE(phycoeval::str_util::pad_int(0, 1) == "0");
        REQUIRE(phycoeval::str_util::pad_int(0, 2) == "00");
        REQUIRE(phycoeval::str_util::pad_int(0, 3) == "000");
        REQUIRE(phycoeval::str_util::pad_int(1, 3) == "001");
        REQUIRE(phycoeval::str_util::pad_int(11, 3) == "011");
        REQUIRE(phycoeval::str_util::pad_int(111, 3) == "111");
        REQUIRE(phycoeval::str_util::pad_int(1111, 3) == "1111");
        REQUIRE(phycoeval::str_util::pad_int(1111, 4) == "1111");
        REQUIRE(phycoeval::str_util::pad_int(1111, 5) == "01111");
    }
}

TEST_CASE("Testing get_indent", "[string_util]") {
    SECTION("Testing get_indent") {
        REQUIRE(phycoeval::str_util::get_indent() == "    ");
        REQUIRE(phycoeval::str_util::get_indent(0) == "");
        REQUIRE(phycoeval::str_util::get_indent(1) == "    ");
        REQUIRE(phycoeval::str_util::get_indent(2) == "        ");
        REQUIRE(phycoeval::str_util::get_indent(3) == "            ");
    }
}
...
TEST_CASE("Testing lstrip", "[string_util]") {
    SECTION("Testing all white space") {
        std::string s = "\n\t   \t\n";
        std::string t = phycoeval::str_util::lstrip(s);
        REQUIRE(s == "\n\t   \t\n");
        REQUIRE(t == "");
    }
...
TEST_CASE("Testing rstrip", "[string_util]") {
    SECTION("Testing all white space") {
        std::string s = "\n\t   \t\n";
        std::string t = phycoeval::str_util::rstrip(s);
        REQUIRE(s == "\n\t   \t\n");
        REQUIRE(t == "");
    }
...
TEST_CASE("Testing strip", "[string_util]") {
    SECTION("Testing all white space") {
        std::string s = "\n\t   \t\n";
        std::string t = phycoeval::str_util::strip(s);
        REQUIRE(s == "\n\t   \t\n");
        REQUIRE(t == "");
    }
...
TEST_CASE("Testing join", "[string_util]") {
    ...
    std::string s = phycoeval::str_util::join(strings, "---");
    REQUIRE(s == "1---2---3");
}
...
TEST_CASE("Testing startswith", "[string_util]") {
    SECTION("Testing startswith") {
        REQUIRE(phycoeval::str_util::startswith("amalie", "amalie") == true);
        REQUIRE(phycoeval::str_util::startswith("amalie", "amali") == true);
        REQUIRE(phycoeval::str_util::startswith("amalie", "a") == true);
        REQUIRE(phycoeval::str_util::startswith("amaliee", "amalie") == true);
        REQUIRE(phycoeval::str_util::startswith("amalie", "amaliee") == false);
        REQUIRE(phycoeval::str_util::startswith("amalie", "A") == false);
        REQUIRE(phycoeval::str_util::startswith("amalie", "eden") == false);
    }
}
...
TEST_CASE("Testing parse_map", "[string_util]") {
    SECTION("Testing parse_map") {
        ...
        std::map<std::string, std::string> returned_map = phycoeval::str_util::parse_map(s, ',', '=');
    ...
    SECTION("Testing parse_map with adding to map") {
        ...
        for (auto s : v) {
            phycoeval::str_util::parse_map(s, returned_map, ',', '=');
        }
        REQUIRE(returned_map == expected_map);
    }
}
