...
namespace namu {
    ...
    inline void Likelihood::set_model(Model::SharedPtr m) {
        assert(_instances.size() == 0); // can't change model after init_beagle_lib called
        _model = m;
    }

    inline void Likelihood::set_rooted(bool is_rooted) {
        assert(_instances.size() == 0 || _rooted == is_rooted); // can't change rooting status after init_beagle_lib called
        _rooted = is_rooted;
    }
    ...
    inline void Likelihood::init_beagle_lib() {
        ...
        _ntaxa = _data->get_num_taxa();

        unsigned nsubsets = _data->get_num_subsets();
        ...
        for (unsigned subset = 0; subset < nsubsets; subset++) {
            ...
            unsigned nstates = _data->get_num_states_for_subset(subset);
        ...
        for (auto p : nstates_ncateg_combinations) {
            new_instance(p.first, p.second, subsets_for_pair[p]);
        ...
        if (_ambiguity_equals_missing)
            ...
            set_tip_partials();
        set_pattern_weights();
        set_pattern_partition_assignments();
    }
    ...
    inline void Likelihood::new_instance(unsigned nstates, int nrates, std::vector<unsigned> & subset_indices) {
        ...
        for (auto s : subset_indices) {
            num_patterns += _data->get_num_patterns_in_subset(s);
        }

        unsigned num_internals = calc_num_internals_in_fully_resolved_tree();

        // add 1 to num_edges so that subroot node will have a tmatrix, root tip's tmatrix is never used
        unsigned num_edges = calc_num_edges_in_fully_resolved_tree();
        ...
        if (inst < 0) {
            ...
            throw NamuX(boost::str(boost::format("Likelihood init function failed to create BeagleLib instance (BeagleLib error code was %d)") % _beagle_error[inst]));
        }
    ...
    inline void Likelihood::set_tip_states() {
        ...
        for (auto & info : _instances) {
            ...
            unsigned t = 0;
            for (auto & row : _data->get_data_matrix()) {
            ...
            if (code != 0)
                throw NamuX(boost::format("failed to set tip state for taxon %d (\"%s\"; BeagleLib error code was %d)") % (t+1) % _data->get_taxon_names()[t] % code % _beagle_error[code]);
            ++t;
            }
        }
    }
    ...
    inline void Likelihood::set_pattern_partition_assignments() {
        ...
        for (auto & info : _instances) {
            ...
            if (code != 0) {
                throw NamuX(boost::format("failed to set pattern partition. BeagleLib error code was %d (%s)") % code % _beagle_error[code]);
            }
        }
    }
    ...
    inline void Likelihood::set_pattern_weights() {
        ...
        auto pattern_counts = _data->get_pattern_counts();
        ...
        for (auto & info : _instances) {
            ...
            if (code != 0)
                throw NamuX(boost::format("Failed to set pattern weights for instance %d. BeagleLib error code was %d (%s)") % info.handle % code % _beagle_error[code]);
        }
    }
    ...
    inline void Likelihood::define_operations(Tree::SharedPtr t) {
        assert(_instances.size() > 0);
        assert(t);
        assert(t->is_rooted() == _rooted);

        _relrate_normalizing_constant = _model->calc_normalizing_constant_for_subset_rel_rates();
        ...
        for (auto nd : boost::adaptors::reverse(t->_levelorder)) {
            ...
            if (!nd->_left_child) {
                // This is a leaf
                queue_t_matrix_recalculation(nd);
            }
            else {
                ...
                assert(rchild);
                queue_partials_recalculation(nd, lchild, rchild);
            }
        }
    }

    inline void Likelihood::queue_partials_recalculation(Node * nd, Node * lchild, Node * rchild) {
        for (auto & info : _instances) {
            unsigned instance_specific_subset_index = 0;
            for (unsigned s : info.subsets) {
                add_operation(info, nd, lchild, rchild, instance_specific_subset_index);
                ++instance_specific_subset_index;
            }
        }
    }   //end_queue_partials_recalculation
    ...
    inline void Likelihood::queue_t_matrix_recalculation(Node * nd) {
        ...
        for (auto & info : _instances) {
            ...
            for (unsigned s : info.subsets) {
                ...
                unsigned tindex = get_t_matrix_index(nd, info, instance_specific_subset_index);
    ...
    inline void Likelihood::add_operation(InstanceInfo & info, Node * nd, Node * lchild, Node * rchild, unsigned subset_index) {
        ...
        if (info.subsets.size() > 1) {
            ...
            _operations[info.handle].push_back(BEAGLE_OP_NONE); // accumulate in calc_instance_log_likelihood
        }
    }
    ...
    inline void Likelihood::update_transition_matrices() {
        ...
        for (auto & info : _instances) {
            ...
            if (code != 0)
                throw NamuX(boost::str(boost::format("Failed to update transition matrices for instance %d. BeagleLib error code was %d (%s)") % info.handle % code % _beagle_error[code]));
    ...
    inline void Likelihood::calculate_partials() {
        ...
        for (auto & info : _instances) {
            ...
            if (nsubsets > 1) {
                ...
                if (code != 0)
                    throw NamuX(boost::format("failed to update partials. BeagleLib error code was %d (%s)") % code % _beagle_error[code]);
            }
            else {
    ...
    inline double Likelihood::calc_instance_log_likelihood(InstanceInfo & info, Tree::SharedPtr t) {
        ...
        int child_partials_index   = get_partial_index(t->_root, info);
        int parent_partials_index  = get_partial_index(t->_preorder[0], info);
        int parent_tmatrix_index   = get_t_matrix_index(t->_preorder[0], info, 0);
        ...
        if (_underflow_scaling) {
            ...
            if (nsubsets == 1) {
                ...
                if (code != 0)
                    throw NamuX(boost::str(boost::format("failed to reset scale factors in calc_instance_log_likelihood. BeagleLib error code was %d (%s)") % code % _beagle_error[code]));
                ...
                if (code != 0)
                    throw NamuX(boost::str(boost::format("failed to accumulate scale factors in calc_instance_log_likelihood. BeagleLib error code was %d (%s)") % code % _beagle_error[code]));
            }
            else {
                for (unsigned s = 0; s < nsubsets; ++s) {
                    code = beagleResetScaleFactorsByPartition(info.handle, cumulative_scale_index, s);
                    if (code != 0)
                        throw NamuX(boost::str(boost::format("failed to reset scale factors for subset %d in calc_instance_log_likelihood. BeagleLib error code was %d (%s)") % s % code % _beagle_error[code]));
                    ...
                    code = beagleAccumulateScaleFactorsByPartition(
                        ...
                        s);
                    if (code != 0)
                        throw NamuX(boost::str(boost::format("failed to acccumulate scale factors for subset %d in calc_instance_log_likelihood. BeagleLib error code was %d (%s)") % s % code % _beagle_error[code]));
                }
            }
        }
        ...
        if (nsubsets > 1) {
            ...
            for (unsigned s = 0; s < nsubsets; s++) {
                ...
                _subset_indices[s]  = s;
                _freqs_indices[s]   = s;
                _tmatrix_indices[s] = get_t_matrix_index(t->_preorder[0], info, s); //index_focal_child + s*tmatrix_skip;
            }
        ...
        if (code != 0) {
            throw NamuX(boost::str(boost::format("failed to calculate edge log-likelihoods in calc_instance_log_likelihood. BeagleLib error code was %d (%s)") % code % _beagle_error[code]));
        }
    ...
    inline double Likelihood::calc_log_likelihood(Tree::SharedPtr t) {
        ...
        set_model_rate_matrix();
        set_among_site_rate_heterogeneity();
        define_operations(t);
        ...
        double log_likelihood = 0.0;
        for (auto & info : _instances) {
            log_likelihood += calc_instance_log_likelihood(info, t);
        }
