...
namespace namu {
    ...
    class Split {
        public:
            ...
            split_unit_t                                        get_bits(unsigned unit_index) const;
            bool                                                get_bit_at(unsigned leaf_index) const;
            void                                                set_bit_at(unsigned leaf_index);
            void                                                add_split(const Split & other);

            bool                                                is_equivalent(const Split & other) const;
            bool                                                is_compatible(const Split & other) const;
            bool                                                conflicts_with(const Split & other) const;

            std::string                                         create_pattern_representation() const;
    ...
    inline void Split::set_bit_at(unsigned leaf_index) { 
    ...
    inline Split::split_unit_t Split::get_bits(unsigned unit_index) const { 
        assert(unit_index < _bits.size());
        return _bits[unit_index];
    }

    inline bool Split::get_bit_at(unsigned leaf_index) const { 
        unsigned unit_index = leaf_index/_bits_per_unit;
        unsigned bit_index = leaf_index - unit_index*_bits_per_unit;
        split_unit_t unity = 1;
        split_unit_t bit_to_set = unity << bit_index;
    ...
    inline std::string Split::create_pattern_representation() const { 
        std::string s;
    ...
    inline bool Split::is_equivalent(const Split & other) const { 
        unsigned nunits = (unsigned)_bits.size();
        assert(nunits > 0);
